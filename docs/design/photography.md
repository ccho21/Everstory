...
| Instagram feed (권장) | 4:5 | 1080×1350 | JPG | 85% |
| Instagram 1:1 | 1:1 | 1080×1080 | JPG | 85% |

원본은 ProRAW (iPhone Pro 모델) 또는 HEIF, 편집 후 JPG export.

## Instagram 업로드 (화질 깨짐 방지)

> 인스타는 업로드본을 **무조건 재압축**(JPEG 품질 ~70–75 수준) + 가로 1080px 초과 시 다운스케일 + **모든 색을 sRGB 로 강제 변환**한다. "내 폰에선 괜찮은데 남 폰에선 더 별로"의 주범은 **색공간** — iPhone 의 Display P3 로 내보내면 인스타가 sRGB 로 변환하며 색(특히 red/orange)이 틀어져 칙칙해 보인다. 카메라 화질 문제가 아니라 export·업로드 파이프라인 문제다.

### Export 체크리스트
| 항목 | 값 | 이유 |
|------|----|------|
| 비율 / 사이즈 | 4:5 → **1080×1350** (또는 1:1 1080×1080) | 인스타 재다운스케일 방지 |
| 색공간 | **sRGB ("Convert to sRGB" 필수)** | P3/AdobeRGB 면 색 틀어짐 → 남 폰에서 칙칙 |
| 포맷 | JPEG | |
| 품질 | **85%** (80–90 sweet spot, 100% 금지) | 100% = 큰 파일 → 인스타가 더 세게 재압축 → 오히려 더 나빠짐 |
| 파일 크기 | **< 3.6 MB** | 초과 시 heavier 압축 트리거 |
| 샤프닝 | **Low / 최소** (Screen, ~20–30% 이하) | 인스타 자체 샤프닝과 겹치면 halo·아티팩트 |

### 업로드 체크리스트
- **앱 업로드**: 설정 → Account → **Data Usage → "Upload at Highest Quality" ON**, **Data Saver OFF**. (기본 OFF라 꼭 켤 것 — 특히 Android.) 화질 최우선이면 이 경로가 가장 안전.
- **데스크톱 / Meta Business Suite**: 예약·편의엔 좋지만 위 "고화질 업로드" 토글이 **웹엔 없음**. 위 규격대로 미리 뽑아 올리면 웹도 무난.
- 업로드 전 **메신저/이메일로 주고받지 말 것** (그 과정에서 재압축됨). 원본 파일을 그대로 옮겨 업로드.

### 촬영·콘텐츠
- A5 시트 전체를 한 컷에 욱여넣지 말고 **클로즈업/부분 크롭** — 잔디테일이 1080px 안에서 뭉개지는 걸 방지.
- **글레어·노이즈 최소화** — 깨끗·저노이즈 이미지가 압축도 깔끔하게 됨 (노이즈 많으면 아티팩트 심화).
