...
- 과거 승인 workflow와 변경 이력

## Document Purpose Table

| 문서 | 역할 | 중요도 | 처리 |
|------|------|--------|------|
| `brand.md` | 디자인 SOT. 브랜드 톤, 워드마크, typography, color, layout, sheet footer 기준을 둔다. | P0 | 유지 |
| `voice.md` | 카피 톤 운영 규칙. 상품/웹/이메일 문장이 통과해야 할 기준을 둔다. | P0 | 유지 |
| `pages.md` | Shopify 페이지 구조와 SEO/H1/CTA 기준. 실제 본문은 `docs/shopify/` 에 둔다. | P0 | 유지 |
| `photography.md` | 촬영 디렉션. 제품 사진, lifestyle, about 이미지 제작 기준. | P1 | 유지 |
| `components.md` | UI component rationale. 실제 Liquid spec이 생기기 전까지 의도만 보존. | P1 | 유지 |
| `tokens.json` | 인쇄·웹 색상/타입/레이아웃 값. 사람이 읽는 설명이 아니라 구현용 값. | P1 | 유지 |
