...
**실사용 매핑 확인란 — D-8 미결정(2026-09-21).** 위 인보이스의 구매 기록은 그대로 보존한다. 판매 재질명과 현재 쓰는 제품이 같다고 추정해 연결하지 않는다. 사업주 확인과 제품/구매 근거를 받은 뒤 채운다.

| 현재 판매/공정 | 실사용 제품명·코드 | 확인 근거·일자 |
|---|---|---|
| White Matte | — | 사업주 입력 대기 |
| Silver | — | 사업주 입력 대기 |
| Gold | — | 사업주 입력 대기 |
| Translucent | — | 사업주 입력 대기 |
| 라미네이트 | — | 사업주 입력 대기 |

Materials Guide의 `LAMat-AF or Oraguard`와 위 구매 기록의 차이는 이 매핑을 확인한 뒤 고객 문장 수정 여부를 결정한다. Judge.me는 2026-09-21 Admin에서 **Free Plan**으로 확인했다. 청구 내역은 확인하지 않았으므로 청구액이나 유료 구독 항목을 추정해 추가하지 않는다. 기능 설명은 [Shopify 계획](../shopify/plan.md)을 따른다.
