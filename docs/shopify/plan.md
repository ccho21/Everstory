Everstory — Style Guide 확장 + Shopify MVP 사이트 빌드
Context
왜 지금: 인쇄 시트 brand foundation (폰트 3종 / 컬러 5색 / 톤 / 태그라인 / 차별화 5축) 은 이미 design/ 에 lock 되어 있음. 다음 단계는 이걸 web 채널로 확장해 Shopify 에서 9 SKU (Solo / Duo / Trio / Memory Pack × 6 사이즈 + Mixed) 의 첫 매출을 받는 것. 현재 design/storefront.md 는 TODO, 웹 전용 토큰·컴포넌트·카피·페이지 구조가 모두 미정.
...
A.1 Web typography 명세 — design/brand_identity.md 에 §Web 섹션 추가
Display / wordmark: SVG 변환 (assets/online_logo.png → design/wordmark.svg)
...
A.2 Web color 토큰 확장 — design/tokens.json 에 web 키 추가
...
A.3 Spacing & layout 시스템 — design/brand_identity.md 에 §Layout 추가
...
A.4 Voice & copy 가이드 — design/voice.md 신규
...
A.5 Photography direction — design/photography.md 신규
iPhone 추가 촬영 샷리스트:
...
prose spec 은 design/components.md 에 rationale 만 (왜 black 버튼인지 등). spec 자체는 Liquid schema 가 source of truth. 다음 컴포넌트만 정의:
...
design/brand_identity.md — Web 섹션 추가 (typography / layout / spacing)
design/tokens.json — web 키 + print 키로 분리, web 토큰 셋 추가
design/decisions.md — ADR-0006 (web color extension), ADR-0007 (component spec lives in Liquid schema)
design/voice.md — 신규
design/photography.md — 신규
design/components.md — 신규 (rationale only, spec 은 Liquid)
design/storefront.md — 페이지별 카피 / CTA / 제목
design/wordmark.svg — 신규 (assets/online_logo.png 변환)
...
docs/business_strategy.md §8 (Shopify 우선), §9 (W1-W6 일정), §13 (HST)
docs/product_mvp_photo_sheet.md — 사이즈 / 재질 / cap 표
