...
## 7️⃣ 실제 batch 6 에 사용된 프롬프트 (history)

### 첫 요청

```
포토샵누끼/docs/shopify/instructions 배치 6 하나하나 따라가게 인스트럭션 만들어주자. 최대한 참조하라 이런거 없이 풀 텍스트로 보여주고, 밸류들 넣을거 카피 하기 쉽게 만들어줘. 또한 지금 horizon 테마 버전은, 3.5.1 이니까 이거에 맞는 UI 랑 설명
```

### 후속 요청 (Theme Editor 단계별 안내로 더 잘게)

```
그렇다면 theme editor 에서 작업할수있게 하나하나 한단계한단계 쉽게 설명해줘. 최신 UI 반영해서
```

이 두 요청을 합친 것이 위 2️⃣ full 프롬프트의 원형. 한 번에 1️⃣ 또는 2️⃣ 로 던지면 두 번 왔다갔다 안 해도 됨.

---

## 8️⃣ 프롬프트가 잘 작동하지 않을 때
