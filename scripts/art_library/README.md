...
|---|---|---|
| `알파벳 샘플_6.ai` | `alphabet_art_v2.ai` | 그룹 `LTR A` ~ `LTR Z` |
...
원본을 고쳤으면 **Illustrator → File → Scripts → Other Script → `build_bubble_style.jsx`**.
원본은 작업 폴더(임시)에 사본으로만 열고 저장하지 않는다. 다 만들면 요약 창이 뜨고 **"예"를 눌러야** `templates` 에 덮어쓴다.
원본이 기본 위치에 없으면 파일 고르기 창이 뜬다. 1분 안팎 (2026-09-17 실측 64초).
...
- **글자 그룹** = 몸통 `BODY`(검정 테두리 컴파운드 — 색 12,12,12 · 넓이 4000 이상인 것 26개를 위→아래, 왼→오른 순으로 A~Z)
  + 안쪽 장식 + `FILL`(흰 채우기) + 맨 아래 `SIL`(조각 전체의 합집합 바깥 윤곽, 흰색).
...
- 미리보기 그림 = 그룹 경계 그대로(투명 PNG, 글자 높이 240px · 두들 200px). 옆 장식이 있는 글자는 `LTR_C_R.png` 처럼 한 장 더.
