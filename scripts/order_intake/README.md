...
**NAME 수동 권고(D-11 미결정):** 후보 5–7장 중 최종 5장을 먼저 고르고 그 사진만 누끼한다. 현재 진행 열은 원본 수를 분모로 쓰므로 `5/7`로 남을 수 있다. 시트 존재만으로 완료를 판단하지 말고 최종 5디자인·이름/스타일·A5 1장·주문 수량/Extra sheets를 대조한다. 저장 순번(`max+1`)은 원본 번호와 다를 수 있다. 진행률 정책을 바꾼 것은 아니다([컷오버 런북](../../docs/business/cutover_runbook.md) §12).
...
python3 composed_preview_test.py   # 구성 미리보기 — 엔진 추출(node 교차 실행 · 배치 선택/크기 직접/버블 이름 포함)·이름 스타일 키·글자/데코 그림 목록과 경로·경로 조작 차단·캐시·폴더 이름 주문번호·만들기 요청(배치·크기·이름 스타일 검사)·HTTP
```

`composed_preview_test.py` 는 임시 폴더에 합성 PNG와 정상 비압축 RGB PSD를 직접 만들고, 127.0.0.1 임시 서버로 라우트까지 돈다.
PSD 생성은 `sips` 인코더에 의존하지 않는다. 썸네일은 운영 함수의 실제 PSD→PNG 변환을 검사하며,
변환 실패는 FAIL로 남기고 후반 검사도 계속한다. `sips`가 없으면 해당 검사 묶음을 건너뛴 것으로 따로 표시한다.
직접 호출과 HTTP 핸들러의 캐시는 모두 임시 폴더를 사용하고, 종료 시 서버와 임시 파일을 정리한다.
