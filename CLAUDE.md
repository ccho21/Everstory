...
│   ├── alphabet_art_v2.ai         # range.jsx 전용 — 버블 스타일 알파벳 (`LTR A` + `SIL`·`SIDE L/R`, scripts/art_library 로 다시 만든다)
