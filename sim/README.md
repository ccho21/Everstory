...
(181 검사 — 아래 사진 종류·파일명 표시·시트 나누기·주문 보드 미리보기·배치 선택·크기 직접 고르기·이름 스타일·글자 색·데코 돌리기 포함).
...
| 데코 | 12종 순서 (이름 자리부터 — 아래) · 박스 = 그림 | 두들 19종 순서(`DECO_ORDER_V2`, 이름 자리부터) · 박스 크기는 같고 그림은 칼선 여백만큼 작게(`payload.pad`) · 글씨 두들(`DECO_BIG_ONLY_V2`)은 12.7mm 칸에만 |
...
- **레트로 = 작업 전 그대로**: 이름 치수(비트 단위)·배치 지문 `af6aae65`·데코 순서(시작 자리 0 일 때)를 테스트가 스냅숏과 대조한다.
...
- **데코 시작 자리** `_composedDecoStart(heroSpec, sheetIndex)`: 예전에는 시트마다 순서 맨 앞에서 시작해 버블 19종 중 앞 8종,
  레트로 12종 중 앞 6종만 나왔고 여러 장 주문은 시트마다 데코가 같았다. 이제 스티커 이름 글자 해시로 시작 자리를 정하고,
  다음 시트는 앞 시트 구간(`_composedDecoSpan` = 글씨 두들이 아닌 모양이 `COMPOSED_DECO_MAX` 개 드는 가장 짧은 구간 — 작은 칸이
  글씨 두들을 건너뛰어도 한 시트의 데코는 이 안에서 나온다) 바로 뒤에서 이어 간다 → **두 장까지는 시트끼리 데코가 안 겹친다**.
  처음엔 "다음 시트 = +6칸" 으로 했다가 테스트가 겹침 24/48 을 잡았다(작은 칸이 글씨 두들을 건너뛰면 구간이 8~9칸).
  이름·시트 번호만 쓰므로 배치 선택과 상관없이 미리보기와 Illustrator 가 같은 값 — `_composedPackExtras(…, sheetIndex)` ·
  `_composedVariants(…, sheetIndex)` 에 시트 번호를 넘긴다. 288시트 시뮬에서 두들 19종·12종이 모두 나온다.
