...
| 데코 | 12종 순서 (이름 자리부터 — 아래) · 박스 = 그림 | 두들 13종 순서(`DECO_ORDER_V2`, 이름 자리부터) · 박스 크기는 같고 그림은 칼선 여백만큼 작게(`payload.pad`) |
...
- **데코 시작 자리** `_composedDecoStart(heroSpec, sheetIndex)`: 예전에는 시트마다 순서 맨 앞에서 시작해 앞 몇 종만 나왔고
  ...
  (버블 13종 · 레트로 12종). 처음엔 글씨 두들이 작은 칸을 건너뛰어 구간이 8~9칸이라 `_composedDecoSpan` 으로 셌는데
  ...
  `_composedVariants(…, sheetIndex)` 에 시트 번호를 넘긴다. 288시트 시뮬에서 두들 13종·12종 · 말풍선 6종·6종이 모두 나온다.
...
- **실제 Illustrator (2026-09-17, 보드가 보내는 요청 그대로 · Charles 6장 한 장씩)**: 버블(양옆) = BESTDAY · FOREVER 22mm,
