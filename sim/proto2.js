// 프로토타입 C: 고아 행 해소 — 폭 미달 행의 아이템을 다른 행 잔여 폭으로 이주,
//   이주 실패한 반복분은 폐기, 유일본이면 행 유지(누락 금지). "장수보다 빈 구간" 우선.
var fs=require('fs');
var src=fs.readFileSync('packer.js','utf8');
var ANCHOR="    var placed = _shelfRowsToPlaced(rows, binW, binH, gap);";
if(src.indexOf(ANCHOR)<0) throw new Error('앵커 불일치');
var DISSOLVE=[
...
"    }"].join('\n');
fs.writeFileSync('packer_proto2.js',
  'var ORPHAN_ROW_MIN_W = '+(process.env.MINW||0.6)+';\n'+src.replace(ANCHOR,DISSOLVE+'\n'+ANCHOR));
...
function score(P,data,n,label){
  ...
  var orph=0,worst=0,st=0,fills=[],lonely=0;
  plan.sheets.forEach(function(s){
    ...
    r.rows.forEach(function(R){
      var free=(binW-R.w)/MM;
      if(free>25){orph++; if(free>worst)worst=free;}
      if(R.items.length===1 && free>40) lonely++;
    });
  });
  console.log(label.padEnd(20)+' 충전 '+fills.map(function(f){return (f*100).toFixed(0)+'%';}).join('/').padEnd(14)+
    ' 스티커 '+String(st).padStart(3)+'  빈행 '+String(orph).padStart(2)+
    '  외톨이행 '+String(lonely).padStart(2)+'  최악잔여 '+worst.toFixed(0).padStart(3)+'mm  경고 '+plan.warn);
}
var BASE=require('./packer.js'), C=require('./packer_proto2.js');
console.log('══ 누리 25디자인 / 2시트 ══');
score(BASE,D,2,'현재');
score(C,D,2,'proto C ('+(process.env.MINW||0.6)+')');
console.log('\n══ 누리 25디자인 / 3시트 ══');
score(BASE,D,3,'현재');
score(C,D,3,'proto C');
console.log('\n══ Package Full 16디자인 / 2시트 (회귀 확인) ══');
score(BASE,PF,2,'현재');
score(C,PF,2,'proto C');
